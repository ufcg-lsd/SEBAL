...
load("d_sun_earth.RData")					# Ler distancia relativa do Sol a Terra
...
MTL <- read.table(dados$MTL[1], skip=0, nrows=140, sep="=", quote="''", as.is=TRUE) 					# Read the MTL File
fic <- substr(MTL$V2[MTL$V1 == grep(pattern="LANDSAT_SCENE_ID", MTL$V1, value=T)], 3, 23)				# Image name that will be processed
...
Dia.juliano <- as.numeric(substr(fic, 14, 16))   

# Getting the sum elevation at the time of Image Capture     											# Julian Day
...
if (n.sensor==8) fic.st <- stack(as.list(fichs.imagens[c(4:9, 2)]))				#### Fmask and Loading the images from the disk and putting in a stack
...
if (n.sensor != 8) fic.st[Fmask != 672] <- NaN else fic.st[Fmask != 2720] <- NaN
endCluster() # ??
proc.time()
...
################## Calculating the image energy balance ####################################
...
################## Masking landset rasters output #########################################
...
phase2 <- function() {
	...
	endCluster() # endCluster?? whereis begin??
